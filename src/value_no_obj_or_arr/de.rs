                ...
                V: MapAccess<'de>,
            {
                match tri!(visitor.next_key_seed(KeyClassifier)) {
                    ...
                    Some(KeyClass::Number) => {
                        ...
                        Ok(ValueNoObjOrArr::Number(number.value))
                    }
                    #[cfg(feature = "raw_value")]
...
macro_rules! deserialize_number {
    ($method:ident) => {
        ...
        {
            match self {
                ValueNoObjOrArr::Number(n) => n.$method(visitor),
...
fn visit_array<'de, V>(array: Vec<ValueNoObjOrArr>, visitor: V) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
{
    let len = array.len();
...
fn visit_object<'de, V>(object: Map<String, ValueNoObjOrArr>, visitor: V) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
{
    let len = object.len();
...
impl<'de> serde::Deserializer<'de> for ValueNoObjOrArr {
    type Error = Error;
    ...
    {
        match self {
            ValueNoObjOrArr::Null => visitor.visit_none(),
...
impl<'de> IntoDeserializer<'de, Error> for ValueNoObjOrArr {
...
impl<'de> IntoDeserializer<'de, Error> for &'de ValueNoObjOrArr {
    type Deserializer = Self;
...
struct SeqDeserializer {
    iter: vec::IntoIter<ValueNoObjOrArr>,
}

impl SeqDeserializer {
    fn new(vec: Vec<ValueNoObjOrArr>) -> Self {
        SeqDeserializer {
            iter: vec.into_iter(),
        }
    }
...
struct MapDeserializer {
    iter: <Map<String, ValueNoObjOrArr> as IntoIterator>::IntoIter,
    value: Option<ValueNoObjOrArr>,
}

impl MapDeserializer {
    fn new(map: Map<String, ValueNoObjOrArr>) -> Self {
        MapDeserializer {
            iter: map.into_iter(),
            value: None,
        }
...
fn visit_array_ref<'de, V>(array: &'de [ValueNoObjOrArr], visitor: V) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
{
    let len = array.len();
...
fn visit_object_ref<'de, V>(
    object: &'de Map<String, ValueNoObjOrArr>,
    visitor: V,
) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
...
impl<'de> serde::Deserializer<'de> for &'de ValueNoObjOrArr {
    type Error = Error;
...
struct EnumRefDeserializer<'de> {
    variant: &'de str,
    value: Option<&'de ValueNoObjOrArr>,
}
...
struct SeqRefDeserializer<'de> {
    iter: slice::Iter<'de, ValueNoObjOrArr>,
}

impl<'de> SeqRefDeserializer<'de> {
    fn new(slice: &'de [ValueNoObjOrArr]) -> Self {
...
struct MapRefDeserializer<'de> {
    iter: <&'de Map<String, ValueNoObjOrArr> as IntoIterator>::IntoIter,
    value: Option<&'de ValueNoObjOrArr>,
}

impl<'de> MapRefDeserializer<'de> {
    fn new(map: &'de Map<String, ValueNoObjOrArr>) -> Self {
        MapRefDeserializer {
            iter: map.into_iter(),
            value: None,
        }
...
impl ValueNoObjOrArr {
    ...
    fn unexpected(&self) -> Unexpected {
        match self {
            ValueNoObjOrArr::Null => Unexpected::Unit,
